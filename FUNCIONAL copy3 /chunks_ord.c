...
/*   Updated: 2024/02/14 11:55:55 by amile-ge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */            
#include "push_swap.h"
#include "push_swap.h"
        ...
        while(num <= t_particio) 
        {
            while(chunks !=0)
            {
              ...
              //     last_ord(stack_a);
                ...
                smalle_ind = -1;
                while(stack_b->first->index != smalle_ind)
                {  
                    if((smalle_ind=find_ind(stack_a, stack_b,chunks)!=-1))
                    {
                        if (stack_b->first->next->index == smalle_ind)
                            sa(stack_b);
                        else if(cost(stack_b,smalle_ind, 'f')==1)
                            ra(stack_b);
                        else if (cost(stack_b,smalle_ind, 'f')== 0)
                            rra(stack_b);
          ...
          //      if (stack_b->first->index >= t_chunks*num)
           //     {
                    ...
                    if (((len(stack_a) > 2 )&& (check_if_ordered(stack_a)== 1)))
                    {
                        ...
                        {
                            int mal;
                            mal = error_ind(stack_a);
                            if (cost(stack_a, mal, 'f')==1)
                                ra(stack_a);
                            else if (cost(stack_a, mal, 'f')==0)
                                rra(stack_a);
                            sa(stack_a);
